    ...
    <div>
       ...
       <div class="input-group mb-3">
        ...
        <table class="table table-responsive table-bordered text-white text-center order">
            ...
            <tbody>
                <tr scope="row" v-for="(product,index) in order.order_details" :key="index">
                    <th><img :src="'/storage/cover_images/'+product.image" style="width: 50px;height: 50px;"></th>
        ...
        <div class="row">
            <div class="col">
                <h4><span class="badge badge-warning">Có {{order.length}} mặt hàng trong hóa đơn</span></h4>
            </div>
            <div class="col">
        ...
        <div class="row justify-content-center">
            <div class="col-12 col-sm-6 col-md-8">
                ...
                <div class="form-group">
                    ...
                    v-model="searchQuery"
                    @focus="modal = true" @change="removeSelectedCustomer()">
                ...
                <template class="form-group" v-if="searchQuery !== '' && customer.name === '' ">
                    <small>Chưa có khách hàng</small>
                    <br>
                    <div class="form-group">
                        <div class="custom-control custom-radio custom-control-inline text-center">
                            <label class="custom-control-label" for="defaultInline2">Thêm khách hàng</label>
                            <input type="radio" class="custom-control-input" checked id="defaultInline2" name="CustomerRadio" value="themmoi" v-model="addCustomer">
                        </div>
                        <div class="custom-control custom-radio custom-control-inline text-center">
                            <label class="custom-control-label" for="defaultInline1">Không thêm</label>
                            <input type="radio" class="custom-control-input" id="defaultInline1" name="CustomerRadio" checked value="khongthem" v-model="addCustomer">
                        </div>
                    </div>
                </template>
                ...
                <div class="form-group">
                    <a class="form-control btn btn-success" @click="editOrder()"><i class="fa fa-print" aria-hidden="true"></i> Lập hóa đơn</a>
                </div>
            </div>
        </div>
                    ...
                    <table class="table table-bordered text-dark text-center">
                        ...
                        <tbody>
                            ...
                            <tr v-for="(item,index) in order_success.order_details" :key="item.id">
                                ...
                                <td>{{item.discount}} %</td>
                                <td class="text-right">{{  (item.quantity * item.price) - (item.quantity * item.price)*(item.discount / 100) - (item.quantity * item.price)*(order_success.discount / 100) | currency('VND', 0 , { thousandsSeparator: ',' , spaceBetweenAmountAndSymbol:true   ,symbolOnLeft: false}) }}</td>
                            </tr>
                            <!-- <template v-if="order_success.order_details && order_success.order_details.length < 10">
                            <tr v-for="item in (10-order_success.order_details.length)" :key="item">
                                <th height="50">{{ order_success.order_details.length += 1 }}</th>
                                <td></td>
                                <td></td>
                                <td></td>
                                <td></td>
...
export default {
    ...
    computed: {
        ...
        totalPrice(){
            return this.order.order_details.reduce((total, item) => {
                return total + (item.quantity * item.price) - (item.quantity * item.price * item.discount / 100)
            }, 0)
        },
        totalpriceAfterTaxAndDiscount(){
            return this.totalPrice + this.totalPrice * (this.tax / 100) - this.totalPrice * (this.discount / 100)
        },
        resultQuery(){
            if(this.searchQuery){
    ...
    methods: {
        fetchOrder(id){
            ...
            .then((res) => {
                ...
                if(res.data.customer !== null){
                    ...
                    this.searchQuery = res.data.customer.name
                }else {
                    this.searchQuery = res.data.order.name
                }
                this.tax = this.order.tax
                this.tax_code = this.order.tax_code
                this.discount = this.order.discount
                this.kind = this.order.kind
            })
            .catch((error) => {
                console.log('Lấy hóa đơn thất bại!!!')
            })
        },
        fetchCustomers(){
        ...
        addProductToCart(product){
            if(product.price != null){
                ...
                if(this.order.order_details.length === 0){
                    this.order.order_details.push(data)
                }else if(this.order.order_details.length > 0){
                    const cartItem = this.order.order_details.find(order_details => order_details.product_code === product.code)
                    if(cartItem){
        ...
        editOrder(){
            ...
            if(!this.order.order_details.length){
                ...
                if(confirm('Bạn có muốn sửa hóa đơn và thêm khách hàng mới?')){
                    let items = JSON.stringify(this.order.order_details)
                    let data = {
                    ...
                    axios.post('/api/orders/edit2',data, {
                        headers: {
                            "Authorization": 'Bearer '+this.currentUser.token
                        }
                    })
                ...
                if(confirm('Bạn có muốn sửa hóa đơn?')){
                    let items = JSON.stringify(this.order.order_details)
                    let data = {
                        id: this.$route.params.id,
                        kind: this.order.kind,
                    ...
                    axios.post('/api/orders/edit3',data, {
                        headers: {
                            "Authorization": 'Bearer '+this.currentUser.token
                        }
                    })
                ...
                if(confirm('Bạn có muốn sửa hóa đơn với khách hàng: '+this.customer.name+'?')){
                    let items = JSON.stringify(this.order.order_details)
                    let data = {
                        id: this.$route.params.id,
                        kind: this.order.kind,
                    ...
                    axios.post('/api/orders/edit1',data, {
                        headers: {
                            "Authorization": 'Bearer '+this.currentUser.token
                        }
                    })
